...
import static io.github.karlatemp.karframework.utils.StringUtils.*;
...
public final class NetworkUtils {
    ...
    public static HttpURLConnection resolveConnection(HttpURLConnection conn) throws IOException {
        ...
        while (true) {
            ...
            if (code >= 300 && code <= 307 && code != 306 && code != 304) {
                ...
                URL u = new URL(conn.getURL(), encodeLocation(newURL));
                HttpURLConnection redirected = (HttpURLConnection) u.openConnection();
    ...
    public static OpenByteArrayOutputStream doGetB(URL url) throws IOException {
        HttpURLConnection con = createHttpConnection(url);
        con = resolveConnection(con);
    ...
    public static Proxy selectProxy(String uri, ConfigurationNode config) {
        String type = config.getNode("type").getString("system");
        switch (type.toLowerCase()) {
            case "system": {
                return ProxySelector.getDefault().select(
                        URI.create(uri)
                ).stream().findFirst().orElse(Proxy.NO_PROXY);
            }
            case "http": {
                String address = config.getNode("address").getString("localhost");
                int port = config.getNode("port").getInt(1080);
